...
public class FoxtrotTableManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        this.indexStoreManager = mock(IndexStoreManager.class);
        this.tableManager = new FoxtrotTableManager(metadataManager, indexStoreManager, dataStore);
    }
    ...
    public void saveTableNullName() throws Exception {
        ...
        doNothing().when(indexStoreManager).initializeTable(any(String.class));
        try {
    ...
    public void saveTableQueryStoreFailed() throws Exception {
        doNothing().when(metadataManager).save(any(Table.class));
        doNothing().when(dataStore).initializeTable(any(Table.class));
        doThrow(new StoreManagerException(StoreManagerException.ErrorCode.INITIALIZATION_EXCEPTION, "Dummy")).when(indexStoreManager).initializeTable(any(String.class));
        try {
            Table table = new Table();
            table.setName("abcd");
            table.setTtl(10);
